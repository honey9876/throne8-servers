...
class SkillController {
    ...
    /**
     * GET ALL SKILLS (authenticated user's own skills)
     */
     ...
     *
     * Authenticated caller can be any logged-in user, but the data returned
     * always belongs to req.params.userId (the profile being viewed), not
     * to the caller. Archived and hidden (isVisible=false) skills are
     * always excluded from this public view.
     */
    static async getSkillsByUserId(
        ...
        try {
            ...
            LoggerUtil.info('Get skills by userId request', {
                targetUserId: userId,
                requestedBy: req.user.userId,
                correlationId,
            });

            // Public view: never include archived skills, regardless of query params
            const result = await SkillService.getAllSkills(userId, false);

            // Public view: only skills explicitly marked visible are returned
            const visibleSkills = (result.skillsList || []).filter(
            ...
            const responsePayload = {
                ...
                pinnedSkills: visibleSkills.filter((s: any) => s.isPinned),
                regularSkills: visibleSkills.filter((s: any) => !s.isPinned),
            };

            const duration = Date.now() - startTime;
            LoggerUtil.info('Skills fetched for public profile', {
                targetUserId: userId,
                requestedBy: req.user.userId,
                count: responsePayload.total,
                duration,
                correlationId,
            });
            ...
            LoggerUtil.error('Get skills by userId failed', {
                error: error.message,
